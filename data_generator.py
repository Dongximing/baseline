...
# info_dict = {'bug_severity': 7, 'bug_status': 3, 'component': 381, 'priority': 4, 'product':39 , 'version': 18}
...
def read_batch_bugs(batch_bugs, data,test=True):
  ...
  for bug_id in batch_bugs:
    ...
    info.append(info_)
  desc_word = torch.from_numpy(data_padding(desc_word, 500)).type(dtype).to(device)
  short_desc_word = torch.from_numpy(data_padding(short_desc_word, 100)).type(dtype).to(device)
  info = torch.from_numpy(np.array(info)).to(device)
  batch_bugs = dict()
  batch_bugs['info'] = info
  batch_bugs['desc'] = (desc_word)
  batch_bugs['short_desc'] = (short_desc_word)
