...
parser.add_argument('-wv', '--word_vocab', type=int, default=200000)
...
def normalized_data(bug_ids):
  ...
  with open(os.path.join(args.data, 'clear.json'), 'r') as f:
    count = 0
    loop = tqdm(f)
    for line in loop:
      bug = json.loads(line)
